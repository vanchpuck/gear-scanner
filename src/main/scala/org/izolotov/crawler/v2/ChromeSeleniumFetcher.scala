...
class ChromeSeleniumFetcher(driverUrl: String, debuggerUrl: String)(implicit httpClient: CloseableHttpClient) extends Fetcher[SeleniumWebPage, SeleniumFetchingAttempt] {

  override def fetch(target: SeleniumWebPage): SeleniumFetchingAttempt = {
    ...
    val responseListener = new ResponseListener(target.url)

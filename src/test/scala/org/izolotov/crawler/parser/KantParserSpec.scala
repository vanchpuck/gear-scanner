...
class KantParserSpec extends FlatSpec {
  ...
  it should "parse product page with no sale price" in {
    val inStream = this.getClass.getClassLoader.getResourceAsStream("kant-parser/black-diamond-cyborg-full-price.html")
  ...
  it should "parse product page with sale price" in {
    val inStream = this.getClass.getClassLoader.getResourceAsStream("kant-parser/deuter-aircontact-sale.html")
  ...
  it should "not fail if some data required on parsing stage doesn't exist" in {
    val inStream = this.getClass.getClassLoader.getResourceAsStream("kant-parser/black-diamond-cyborg-no-title.html")
    val actual = KantParser.parse("http://kant.ru/crampons.html", inStream, Charset.forName("UTF-8"))
    val expected = Product(
      "http://kant.ru/crampons.html",
      "kant.ru",

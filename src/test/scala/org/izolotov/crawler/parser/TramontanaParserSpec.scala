...
class TramontanaParserSpec extends FlatSpec {
  ...
  it should "parse product page with no sale price" in {
    val inStream = this.getClass.getClassLoader.getResourceAsStream("tramontana-parser/black-diamond-cyborg-clip-full-price.html")
  ...
  it should "parse product page with sale price" in {
    val inStream = this.getClass.getClassLoader.getResourceAsStream("tramontana-parser/grivel-air-tech-gsb-sale.html")
    val actual = TramontanaParser.parse("http://tramontana.ru/crampon.html", inStream, Charset.forName("UTF-8"))
    val expected = new Product(
      "http://tramontana.ru/crampon.html",
      "tramontana.ru",
  ...
  it should "not fail if some data required on parsing stage doesn't exist" in {
    val inStream = this.getClass.getClassLoader.getResourceAsStream("tramontana-parser/black-diamond-cyborg-clip-no-data.html")
    val actual = TramontanaParser.parse("http://tramontana.ru/crampon.html", inStream, Charset.forName("UTF-8"))
    val expected = new Product(
      "http://tramontana.ru/crampon.html",
      "tramontana.ru",

...
class ImageProcessor(addToImageDB: (String, Array[Byte]) => Unit, storeToDB: (CrawlAttempt[S3Image]) => Unit) extends Processor[Array[Byte]] {

  override def process(attempt: CrawlAttempt[Array[Byte]]): Unit = {
